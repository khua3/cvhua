...
class VidstgReader(object):
    ...
    def read(self, filename):
        ...
        word2idx = json.load(
            open("./dataset_scope_gt/vidstg.dic.json", 'r')
            )

        lines = []
        anno_path = "./VidSTG-Dataset/VidSTG"
        splits = filename.split("/")[-2]
        anno_file = "{}/{}.json".format(anno_path, splits)
        annos = json.load(open(anno_file,'r'))
        annos = annos['videos']

        
        for bid, anno in enumerate(annos):
            if bid>=3610*1 and splits=='train':
                break
            lines.append(anno["caption"])
     
        for line in tqdm(enumerate(lines), desc='read'):
            sent = str(line)
            s = sent.strip().split(self.delim)
