...
from winner_spatial.data.reading import NLIReader, PlainTextReader, ConllReader, JSONLReader, PTBReader, COCOReader, FlickrReader, VidstgReader
from winner_spatial.data.batch_iterator import BatchIterator
from winner_spatial.data.embeddings import EmbeddingsReader, UNK_TOKEN
from winner_spatial.data.preprocessing import indexify, build_text_vocab
from winner_spatial.data.preprocessing import synthesize_training_data
from winner_spatial.logging.configuration import get_logger
from winner_spatial.blocks.negative_sampler import NegativeSampler, calculate_freq_dist
...
class ReconstructDataset(object):
    ...
    def initialize(self, options, text_path=None, embeddings_path=None, filter_length=0, data_type=None):
        ...
        elif data_type == 'vidstg':
            reader = VidstgReader(lowercase=options.lowercase, filter_length=filter_length)
        else:    
