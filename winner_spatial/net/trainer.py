...
from winner_spatial.net.utils import ImageEncoder
...
class ContrastiveLoss(torch.nn.Module):
    ...
    def __init__(self, margin=1.0, alpha_contr=0.01, use_contr_ce=False):
        ...
        self.pos_contr = 0.1
        self.pos_info = torch.arange(1, 6)**self.pos_contr
    ...
    def forward(self, batch, diora):
        ...
        pos_info = self.pos_info.to(device)
        all_atten_score_pos = all_atten_score / pos_info[None, None, None, :]
        
        assert all_atten_score_pos is not None
        scores = all_atten_score_pos.max(-1).values # bs*bs*span_len
        # TODO: COCO
...
class VGLoss(torch.nn.Module):
    ...
    def forward(self, batch, vg_atten_score):
        ...
        targets = torch.arange(batch_size, device=phrase_region_scores.device)
...
class Net(nn.Module):
    ...
    def compute_loss(self, batch, neg_samples, info, batch_parse, batch_map):
        ret, loss = {}, []
        ...
        for func_name in self.loss_func_names:
            ...
            # elif 'supervise_loss' in func_name:
            #     subloss, desc = func(batch, diora, batch_map)
    ...
    def forward(self, img_ids, idx2word, batch, image_feats, obj_feats, boxes, obj_cates, neg_samples=None, compute_loss=True, info=None, batch_parse=None, batch_map=None):
        """
        ...
        self.diora(embed_span, embed_word, obj_embed_span, obj_embed_word, batch_map) # Flickr
        ...
        if compute_loss:
            ret, loss = self.compute_loss(batch, neg_samples, info=info, batch_parse=batch_parse, batch_map=batch_map)
        else:
...
class Trainer(object):
    ...
    def run_net(self, batch_map, idx2word=None, compute_loss=True, multigpu=False):
        ...
        out = self.net(img_ids, idx2word, batch, image_feats, obj_feats, boxes, obj_cates, neg_samples=neg_samples, compute_loss=compute_loss, info=info, batch_parse=batch_parse, batch_map=batch_map)
        return out
...
def build_net(options, embeddings=None, random_seed=None):
    ...
    image_encoder = ImageEncoder(input_size=1024, size=size)
