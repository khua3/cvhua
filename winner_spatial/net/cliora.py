...
from winner_spatial.net.utils import *
...
class AttentionHead(nn.Module):
    ...
    def forward(self, h_q, h_k, h_v, temp=1.0):
        ...
        abalation = torch.full_like(atten_prob, 0.2).cuda()
        cxt = torch.bmm(atten_prob, h_v)
        # cxt = torch.bmm(abalation, h_v)
        # print(atten_prob.shape, h_v.shape, cxt.shape)
        return cxt
...
class VLComposeMLP(nn.Module):
    ...
    def leaf_transform(self, x, obj, atten_head, normalize_func):
        ...
        # visual as residule
        # mark here: feature level fusion 
        
        # vidstg feature level fusion lambda 0.1
        # default 1
        h = h + cxt
...
class DioraBase(nn.Module):
    ...
    def outside_pass(self):
        ...
        for level in range(self.length - 2, -1, -1):
            batch_info = BatchInfo(
                batch_size=self.batch_size, length=self.length,
                size=self.size, level=level,)
    ...
    def forward(self, x_span, x_word, obj_embed_span=None, obj_embed_word=None, batch_map=None):
